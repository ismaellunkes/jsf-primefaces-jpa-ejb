...
public class MatriculaDaoImpl implements MatriculaDao {
	...
	public List<Matricula> findByAlunoCurso(Matricula matricula) {
		Query query = em.createQuery(" SELECT m FROM Matricula m WHERE m.curso like :curso AND m.pessoa = :pessoa ");		
		query.setParameter("curso", matricula.getCurso());
		query.setParameter("pessoa", matricula.getPessoa());
		return query.getResultList();
	}
	
	@Override
	public List<Matricula> findByCurso(Matricula matricula) {
		Query query = em.createQuery(" SELECT m FROM Matricula m WHERE UPPER(m.curso) like UPPER(:curso) ");		
		query.setParameter("curso", "%"+matricula.getCurso()+"%");
		return query.getResultList();
	}
